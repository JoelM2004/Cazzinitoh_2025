...
class AchievementsListView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final owned    = all.where((a) => userAchievementIds.contains(a.id)).toList();
    final locked   = all.where((a) => !userAchievementIds.contains(a.id)).toList();
    final sorted   = [...owned, ...locked];
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        final ach     = sorted[index];
        final isOwned = userAchievementIds.contains(ach.id);
        return _AchievementTile(ach: ach, owned: isOwned);
      },
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: owned ? AppColors.purpleAccent.withOpacity(0.5) : Colors.white12,
        ),
      ),
      child: Row(
    ...
    return Row(
      ...
      children: [
        Icon(
          Icons.flag_rounded,
          size: 12,
          color: owned ? AppColors.purpleAccent.withOpacity(0.5) : Colors.white,
        ),
        const SizedBox(width: 4),
        Expanded(
          child: Text(
            ...
            style: TextStyle(
              ...
              color: owned ? AppColors.purpleAccent.withOpacity(0.55) : Colors.white,
            ),
          ),
        ),
      ],
